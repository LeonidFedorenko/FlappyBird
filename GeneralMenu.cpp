...
namespace SoundSonar
{
	...
	void GeneralMenu::Init()
	{
		...
		sf::Window window;
		// измен¤ю позицию окна (относительно рабочего стола)
		window.setPosition(sf::Vector2i(0, 50));
		// получаю специфичный дл¤ операционной системы handle окна чтобы имлементировать вещи, 
		// которые SFML не поддерживает
		sf::WindowHandle handle = window.getSystemHandle();
		window.setTitle("»гра");
		// загружаю рисунок дл¤ фона главного окна из ресурсов
		_data->assetcontrols.LoadTextureImage("Main Menu Background",
			MAIN_MENU_BACKGROUND_FILEPATH);
		// загружаю рисунок дл¤ заголовка главного окна из ресурсов
		_data->assetcontrols.LoadTextureImage("Game Title",
			GAME_TITLE_FILEPATH);
		// загружаю рисунок дл¤ кнопки "»грать" из ресурсов
	...
	void GeneralMenu::HandleInput()
	{
		...
		while (_data->window.pollEvent(event))
		{
			...
			// нажатие кнопки "»грать"
			if (_data->input.IsSpriteClicked(_playButton, sf::Mouse::Left, _data->window))
			{	// провер¤ю нажатие через консоль std::cout << "«апуск игры!" << std::endl;
